...
//            if((M_ChkFlag(SL_MPR_RUNING)!=0)&&(M_ChkFlag(SL_ERRSTOP)==0)&&(M_ChkFlag(SL_SERIESTOP)==0))
	        ...
	        {		       
			   ...
			   if(M_ChkCounter(MAIN_LOOP.cnt_lv_mpwmstop,DELAY_MPWMSTOP)>0)	//0ms LVRT 20100125cpc试验确定
			   		M_ClrFlag(SL_LV_MPWMOUT);	      						//脉冲封掉后延时0ms允许开通SCR
	        } 
	}
	...
	{      
		...
		M_ClrFlag(SL_NPR_PWMOUT);
		M_ClrFlag(SL_MPR_PWMOUT);
		M_ClrFlag(SL_LV_MPWMOUT);	      							//脉冲封掉后延时1ms允许开通SCR
...
//		if(M_ChkFlag(SL_LV_QWORKING)==0) 	NGS_Udq_p_ex = NGS_Udq_p;		//判断LVRT之前网压 20130206
		if(M_ChkFlag(SL_LV_QWORKING)==0) 	NGS_Udq_p_ex = NGS_Udq_pflt;		//判断LVRT之前网压 20130301
...
//					    	GIVE.toqrf  = (int16)_PROSTDBY2;   		//201007BJTULVRT
				...
				if     (GIVE.anglerf > 0)
					GIVE.anglerf = GIVE.anglerf * _stdby02;		//20140503LJD电能质量
		 ...
		 if(M_ChkCounter(MAIN_LOOP.cnt_lv_nprqworking,20)>=0)		//20130225
		 {
			...
			if((_COMMAND2&0x0400)!=0)	//备用10 20111106
			{
				if(M_ChkFlag(SL_LV_STATE)==0)
					temp_npriqrf   = -450; 	//1100V额定网压下网侧无功电流峰值须小于460A
			}
...
//--------20130124----网侧发无功填补跌落初期无功的不足----
			if(M_ChkCounter(MAIN_LOOP.cnt_lv_npriqrf,120)<=0)
			{
//				GIVE.npriqrf = - kq * lv_ctrl5 * 400;
				if((NGS_Udq_epsilon < 30) && (NGS_Udq_p < (0.43 * NGS_Udq_p_ex)))		//只3ph-20及35情况下才发	20130308
...
//			if((NGS_Udq_epsilon > 27) && (NGS_Udq_p < (0.71 * NGS_Udq_p_ex)) && (M_ChkFlag(SL_LV_SCRRUNING)!=0))	//20%/35%所有工况	20130222
			if((NGS_Udq_epsilon > 38) && (PRO.Pgactive_lv>600000) && (NGS_Udq_p < (0.61 * NGS_Udq_p_ex)) && (M_ChkFlag(SL_LV_SCRRUNING)!=0))	//20%/35%所有工况	20130306
			{
...
//				GIVE.npriqrf = - kq_lv * _stdby08;	//网压恢复后网侧填补无功400A 20130228
		...
		else		//20130124
		{
			...
			MAIN_LOOP.cnt_lv_nprqworking = 0;		//20130124网侧发无功避过20ms
		}
...
//----MagnetCurve200909 test_start-----------------------
   		...
   		if(_SC_MSTDBY==0)
   		{
			...
			{								
				...
				else if(MC_DATA.in > 110)	MC_DATA.in = 110;		//输入限幅
...
//----MagnetCurve2013-12-13 ZZJ增加励磁调整内容---end
		...
		temp_var = RUN.mpridrf_var;//2013-12-9ZZJ修改无功指令响应问题
//----20121103-----
//-------机侧无功电流控制BJTULVRT201204-------符号20121129-----------
...
//				if(PRO.Pgactive_lv<600000)
				if((PRO.Pgactive_lv<600000) || (NGS_Udq_p > (NGS_Udq_p_ex * 0.83)))		//加入90%和两相75%的情况	20130303
					Te_feedback_lv = GIVE.toqrf_lv;		//反馈主控转矩维持20130128
				else
...
//				if((NGS_Udq_p>(NGS_Udq_p_ex * 0.85)) && (NGS_Udq_epsilon<9) && M_ChkFlag(SL_LV_STRICTLV)!=0)		//2ph-75排除在外 20130309
//					RUN.mpriqrf_g = RUN.mpriqrf_g * 1.14;
				...
				else if((NGS_Udq_p>(NGS_Udq_p_ex * 0.85)) && (NGS_Udq_epsilon<9) && M_ChkFlag(SL_LV_STRICTLV)!=0)		//2ph-75排除在外 20130309
//					RUN.mpriqrf_g = RUN.mpriqrf_g * 1.14;
...
//					RUN.mpridrf_var	= - kq * 605.0;				//20111201 Ir_varn=116A 1.5MW是Irn=kq*505A
					RUN.mpridrf_var	= - kq * 645.0;				//20111201 Ir_varn=116A 1.5MW是Irn=kq*505A	20130309
...
//				    RUN.mpriqrf_g  = (GRD_Utlv/GRD_Ut) * RUN.toqrf * STAROTRTO / (1.5 * SQRT3 * POLEPAIRES * MPR_Lm * GRD_UN / (314.15926 *  MPR_Ls));   //201205LVRTatZB
			...
			temp_idrfmax = - (int16)_PROSTDBY2;			//无功电流限幅
			if(RUN.mpridrf_var < temp_idrfmax)	RUN.mpridrf_var=temp_idrfmax;	//机侧无功指令限幅
...
//			else
			else if(M_ChkFlag(SL_LV_STRICTLV)!=0)				//网压恢复后不延迟发有功 20130228
			{
...
//---------------检查相序是否正确------------------------------------------------------------------
	if((CAP5.phase < TWOPAI_12 ))  					//当相差误差超过PAI/3时报错
	{
		...
		{
			if(M_ChkCounter(MAIN_LOOP.cnt_phorde,DELAY_PHORDE)>0)	//12s 上电延时判断网侧相序故障 201005atcpc
			M_SetFlag(SL_PHORDE);			//DSP上电后	相序有蟊曛疚恢  20090817
		}
		else
		{
			M_ClrFlag(SL_PHORDE);									// 清零相序有误标志位
...
//	temp  = temp2 * CAP4.radpertb;							//网侧幢浠唤嵌龋慈∧＃迹 ---2PAI）
...
//--------对35%-2ph工况做电压环限幅20130222--------
		if(M_ChkFlag(SL_LV_STATE)!=0)
		{
			if(M_ChkFlag(SL_UNBALANCE)!=0 && (NGS_Udq_epsilon < 50) && M_ChkFlag(SL_LV_SCRRUNING)==0)	//除20%-2ph，其余2ph都限值
			{
