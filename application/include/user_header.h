...
#define	DELAY_EIN	    	1000  	 //衔换刂浦鞫希乐刮蟛僮1s
#define	DELAY_SYSRUN	    1000  	 //上位机控制系统运行/电机试验，防止误操1s
...
#define	DELAY_LVTEST2		1000  	 //1s 2013-12-6改为1s
...
#define SL_IN1_MIANFAN	    	0x96	        //=1 功缁＃=超温障   P4.7
...
#define SL_LV_SCRDONE			0xB5			//=1 投入scr一次20130221
...
struct	MAIN_LOOP{
	...
	int32 cnt4;					//主循环ㄊ奔拇嫫 0ms
	int32 cnt5;					//餮范ㄊ奔拇嫫 00ms
	int32 cnt6;					//主循环定时寄存 000ms
...
//电机参数设定2010atcpc
extern float  	MPR_Ls;	 	    				//定子自感	--单位H
extern float	MPR_Lr;	 	    				//转子自感	--单位H
extern float 	MPR_Lm;	 	    				//定转子互感--单位H
extern float 	MPR_Rs;	     					//定子电阻	--单位ohm
extern float 	MPR_Rr;	    					//转子电阻	--单位ohm
extern float 	SIGMA;							//(1-MPR_Lm*MPR_Lm/(MPR_Ls*MPR_Lr))
extern float	STAROTRTO;       				//定转子变比
extern float	STAROTRTO2;       				//定转子变比的平方
extern float	POLEPAIRES;       				//电机极对数
...
extern struct	TRANS_DATA TRS_MPR_U;   //机侧变流器算脉冲用
extern struct	TRANS_DATA TRS_NPR_U;   //网侧变流器算脉冲
extern struct	TRANS_DATA TRS_NGS_U;   //嗟缤缪
extern struct	TRANS_DATA TRS_STA_I;   //定子电流
extern struct	TRANS_DATA TRS_STA_U;   //定子电压
extern struct	TRANS_DATA TRS_GSTA_U;  //目标定子电压
extern struct	TRANS_DATA TRS_PSTA_U;  //用于编码器位置检测的定子电压
extern struct	TRANS_DATA TRS_PMPR_I;  //用于编码器位置检测的转子电流
extern struct	TRANS_DATA DIP_NPR_U;  //用于功率计算
extern struct	TRANS_DATA DIP_NPR_I;  //用于功率计算
extern struct	TRANS_DATA DIP_STA_U;  //用于功率计算
extern struct	TRANS_DATA DIP_STA_I;  //用于功率计算
extern struct	TRANS_DATA DIP_MPR_I;  //用于功率计算
extern struct	TRANS_DATA TRS_GAC_U;  //用于201011PLL
extern struct	TRANS_DATA TRS_SCR_I;   //机侧crowbar电流BJTULVRT201204

extern struct I_DATA_PLL I_DATA_PLL;

extern struct PI_DATA PI_NPR_Id;				//网侧变流器d轴电流闭环
extern struct PI_DATA PI_NPR_Iq;				//网侧变流器q轴电流闭环
extern struct PI_DATA PI_MPR_Id;				//机侧变髌鱠轴电流闭环
extern struct PI_DATA PI_MPR_Iq;				//机侧变流器q轴电流闭环
extern struct PI_DATA PI_NPR_U;				//网侧变流器电压环
extern struct PI_DATA PI_MPR_U;				//软并网闭环
extern struct PI_DATA PI_STA_Id;				//励磁控制d
extern struct PI_DATA PI_STA_Iq;				//励磁控制q
extern struct PI_DATA PI_DATA_PLL;				//201011PLL
extern struct PI_PARA PI_PARA_DYMI;			//动态宽带宽PI控制BJTULVRT201204
...
//慢速AD处理寄存器
extern Uint16 _OUT_AMUX1;  //用于慢速AD选通信号，每次要选通下一个AD通道，_OUT_AMUX加1，然后送到CPLD去处理，选通16选1芯片
extern Uint16 _OUT_AMUX2;    //用于慢速AD选通信号，每次要选通下一个AD通道，_OUT_AMUX加1，然后送到CPLD去处理，选通16选1芯片

extern struct AD AD;
extern struct ADFINAL ADFINAL;

extern struct AD_DATA AD_OUT_GRD_U;  //电网电压（主断前）
extern struct AD_DATA AD_OUT_STA_U;  //电机定子侧电压 大滤波通道
extern struct AD_DATA AD_OUT_NGS_U;  //电网电压（主断后）
extern struct AD_DATA AD_OUT_NPR_I;  //网侧变流器电流
extern struct AD_DATA AD_OUT_MPR_I;  //机侧变流器电流
extern struct AD_DATA AD_OUT_STA_I;  //定子侧电流
extern struct AD_DATA AD_OUT_NGF_U;  //电网电压（主断后）大滤波通道
extern struct AD_DATA AD_OUT_STAD_U;  //差值
extern struct AD_DATA AD_OUT_SCR_I;  //ACTIVECROWBAR电流
extern struct AD_DATA AD_OUT_DC_I;  //CHOPPER电流

extern float  AD_OUT_UDC; //AD计算后输出的直流电梗╒）
...
extern struct BS_DATA BS_MPR_Id;				//转子侧电流d轴
extern struct BS_DATA BS_MPR_Iq;				//转子侧电流q轴
extern struct BS_DATA BS_STA_Id;				//定子侧电流d轴
extern struct BS_DATA BS_STA_Iq;				//定子侧电流q轴
extern struct BS_DATA BS_NPR_Ud;				//网侧变流器d轴
extern struct BS_DATA BS_NPR_Uq;				//网侧变流器d轴
extern struct BS_DATA BS_NPR_Udn;				//网侧变流器d轴
extern struct BS_DATA BS_NPR_Uqn;				//网侧变流器d轴
...
extern Uint16  _MSG_SCOUT1;												//SCOUT信息级别(前次)
extern Uint16  _MSG_SCOUT2;												//SCOUT信息级别(本次)
extern Uint16  _SY_RTRN;
extern Uint32   POSCNT_TEST;
extern float   DM_imrd;   						//转子励磁电流
extern float   DM_imrq;   						//转子励磁电流
extern float   DM_Lr;							//当前重试次数
extern float   Te_feedback;                    //实际转矩值反馈
extern float   Pnactive;    					//网侧有功功率
extern float   Pnreactive;    					//网侧无功功率
extern float   Pcreactive;

//----------------------------参数变量------------------------------------------------------------
extern Uint16 _MC_TYPE;						//机型参数
extern Uint16 _MC_REV;							//软件版本
extern Uint16 _MC_DATE;						//出厂日期
extern Uint16 _MC_ID;							//产品编号
extern Uint16 _SCIB_ADDR;						//485节点地址
extern Uint16 _PROSTDBY1;						//
extern Uint16 _PROSTDBY2;						//

extern Uint16 _SW_FR;							//开关频率(kHz)
extern Uint16 _DEADTIME;						//脉冲的最小导通时间(us)
extern Uint16 _COMPTIME;						//脉冲的最小导通时间(ms)
extern Uint16 _MINONTIME;						//脉冲的最小导通时间(us)

extern Uint16 _RU_UDCT;						//电压指令积分时间(ms)
extern Uint16 _RU_IDT;							//电流指令积分时间(ms)
extern Uint16 _RU_IQT;							//电流指令积分时间(ms)
extern Uint16 _RU_TOQT;						//转矩指令积分变(ms)
extern Uint16 _RU_AGLT;                        //功率因数角指令积分时间(ms)
extern Uint16 _URF;							//直流电压给定值
extern Uint16 _MIDRF;							//d轴电流给定值
extern Uint16 _NIQRF;							//q轴电流给定值
extern Uint16 _MIQRF;							//q轴电流给定值
extern Uint16 _TOQRF;							//转矩指令
extern Uint16 _AGLRF;							//机侧功率因数角
extern Uint16 _EIDCO;							//手动 机侧励磁电流调整系数
extern Uint16 _ENCODPOS;						//手动 编码器初始位置角度

extern Uint16 _STDBY1;							//备用1
extern Uint16 _STDBY2;							//备用2
extern Uint16 _STDBY3;							//备用3
extern Uint16 _STDBY4;							//备用4
...
extern Uint16 _STDBY11;						//备用11
extern Uint16 _STDBY12;						//备用12
extern Uint16 _STDBY13;						//备用13
extern Uint16 _STDBY14;						//备用14
extern float _eidco;
...
extern float _stdby01;							//备用经过小数点处理后值
extern float _stdby02;							//备用经过小数点处理后值
extern float _stdby03;							//赣经过小数点处砗笾
extern float _stdby04;							//备用∈愦砗笾
extern float _stdby05;							//备用经过小数处理后值
...
//-----------------------PI环参数变量--------------------------------------------------------------
extern Uint16	_NPR_U_Kp;		  		//网侧变流器电压闭环比例系数
extern Uint16	_NPR_U_Ki;		   		//网侧淞髌鞯缪贡栈坊窒凳
extern Uint16	_NPR_U_Kd;		   		//网侧澉器电压闭肺⒎窒凳
extern Uint16	_NPR_U_outmax;			//网侧变流器缪贡栈肥涑鱿薹
extern Uint16	_NPR_U_errmax;		 	//网侧变流器电压闭环误差限幅
extern Uint16	_NPR_U_errmin;		   	//网侧变流鞯缪贡栈肺蟛铑兄
extern Uint16	_NPR_U_incrementmax;	//网侧变流器电压闭环增量限幅

extern Uint16	_NPR_ID_Kp;				//网侧变流器电流闭环比例系数
extern Uint16	_NPR_ID_Ki;	   			//网侧变流器电流闭环积分系数
extern Uint16	_NPR_ID_Kd;	   			//网侧变流器电流闭环微分系数
extern Uint16	_NPR_ID_outmax;			//网侧变流器电流闭环输鱿薹
extern Uint16	_NPR_ID_errmax;	 		//网侧变流器电流闭环误差限幅
extern Uint16	_NPR_ID_errmin;	  		//网侧变流器电流闭环误差钚≈
extern Uint16	_NPR_ID_incrementmax; 	//网侧变流器电流闭环隽肯薹

extern Uint16	_NPR_IQ_Kp;				//网侧变流器电流闭环比例系数
extern Uint16	_NPR_IQ_Ki;	   			//网侧变髌鞯缌鞅栈坊窒凳
extern Uint16	_NPR_IQ_Kd;	   			//网啾淞髌鞯缌鞅栈肺⒎窒凳
extern Uint16	_NPR_IQ_outmax;			//网侧变流器电流闭环输出限幅
extern Uint16	_NPR_IQ_errmax;	 		//网侧变流器电流闭环误差限幅
extern Uint16	_NPR_IQ_errmin;	  		//网啾淞髌鞯缌鞅环误差最小值
extern Uint16	_NPR_IQ_incrementmax; 	//网侧变流器电流闭环增量限幅

extern Uint16	_MPR_U_Kp;	   			//机侧变流器电压闭环比例系数
extern Uint16	_MPR_U_Ki;	  			//机侧变流器电压闭环积分系数
extern Uint16	_MPR_U_Kd;	  			//机侧变流器电压闭环微分系数
extern Uint16	_MPR_U_outmax;	 		//机侧变流器电压闭环输出限幅
extern Uint16	_MPR_U_errmax;	  		//机侧变流器电压闭环误差限
...
extern Uint16	_DYN_U_Kd;	  			//机侧变流器电压闭环微分系数
extern Uint16	_DYN_U_outmax;	 		//机侧变流器电压闭环输出限幅
extern Uint16	_DYN_U_errmax;	  		//机侧变流器电贡环误差限幅
extern Uint16	_DYN_U_errmin;		 	//机侧变流器电压闭环误差最小值
extern Uint16	_DYN_U_incrementmax; 	//机侧变流器电贡栈吩隽肯薹


extern Uint16 _SC_RTRN;													//故障最大重试次数
extern Uint16 _SC_RTRT;													//两次故障重试间隔时间(ms)

extern Uint16 _SC_IACOV1;													//基准电流的百分值)
...
extern Uint16 _SC_IACOV2;													//基准电流的百分值)
extern Uint16 _SC_PGOV;													//
extern Uint16 _SC_QGOV;													//
extern Uint16 _SC_IACOVST;                                                 //定子电流过流保护

extern Uint16 _SC_Rs;														//定子电阻(mohm)
extern Uint16 _SC_Lls;														//定子漏抗(mH)
extern Uint16 _SC_Rr;														//转子电阻(mohm)
extern Uint16 _SC_Llr;														//转子漏抗(mH)
extern Uint16 _SC_Lm;														//励磁电 mH)
extern Uint16 _SC_POLES;													//极对数
extern Uint16 _SC_Usn;														//定子额定电压(V)
extern Uint16 _SC_Uro;                                                		//转子开路电压(V)
extern Uint16 _SC_PLSPRVL;													//编码器每圈光栅数量
extern Uint16 _SC_MSTDBY;                                               	//电机参数备用
extern Uint16 _SC_EXISTY1;                                               	//电机励磁调节备用1
extern Uint16 _SC_EXISTY2;                                               	//电机励磁调节备用2
extern Uint16 _SC_EXISTY3;                                               	//电机励磁调节备用3
extern Uint16 _SC_EXISTY4;                                               	//电机励磁调节备用4
extern Uint16 _SC_EXISTY5;                                               	//电机励磁调节备用5

extern Uint16 _BA_ERR4;													//第四次故障信息
extern Uint16 _BA_ERR3;													//第三次故障信息
extern Uint16 _BA_ERR2;													//第二次故障信息
extern Uint16 _BA_ERR1;													//第一次故障信息
extern Uint16 _BA_ERR5;													//示波器故障信息
//--------------------------------报警信息变 ----------------------------------------------------
extern Uint16 _BA_EMIDRF;													//最新故障时刻 d轴电流指令(A)
extern Uint16 _BA_ENIQRF;													//最新故障时刻 q轴电流指令(A)
extern Uint16 _BA_EMIQRF;													//最鹿障时刻 q轴电流指令(A)
extern Uint16 _BA_ETOQRF;													//最新故障时刻 转矩指令(NM)
extern Uint16 _BA_EURF;													//最新故障时刻 中间直流电压给 V)
extern Uint16 _BA_EUDC;													//最新故障时刻 中间直流电压(V)
extern Uint16 _BA_EAGLRF;													//最新故障时刻 无功角度指令()
extern Uint16 _BA_EIA1;													//最新故障时刻 网侧A相电流有效值(A)
extern Uint16 _BA_EIB1;													//最新故障时刻 网侧B相电流有效值(A)
extern Uint16 _BA_EIC1;													//最新故障时刻 网侧C相电流有效值(A)
extern Uint16 _BA_EIA2;													//最新故障时刻 缁郃相电流有效值(A)
extern Uint16 _BA_EIB2;													//最新故障时刻 电机侧B相电流有效值(A)
extern Uint16 _BA_EIC2;													//最新故障时刻 电机侧C相电流有效值(A)
extern Uint16 _BA_EUAB1;													//最新故障时刻 网侧A相电压有效值(V)
extern Uint16 _BA_EUBC1;													//最新故障时刻 网侧B相电压有效值(V)
extern Uint16 _BA_EUAB2;													//最新故障时刻 网侧相电压有效值(V)
extern Uint16 _BA_EUBC2;													//最新故障时刻 电机侧A相电流有效值(V)
extern Uint16 _BA_EUAB0;													//最新故障时刻 主断前AB线电压有效值(V)
extern Uint16 _BA_EUBC0;													//最新故障时刻 主锨癇C线电压有效值(V)
extern Uint16 _BA_EFREQ;													//最新故障时刻 电网频率(hz)
extern Uint16 _BA_ESPEED;													//最新故障时刻 电机转速(rpm)
extern Uint16 _BA_ENPRUD;              			//d轴主断后网压反馈20091026atzy
extern Uint16 _BA_ENPRUQ;          				//q轴主断后网压反馈20091026atzy
extern Uint16 _BA_ENPRUD2;          				//d轴主断后网压反馈 滤波后20091026atzy
extern Uint16 _BA_ENPRUQ2;          				//q轴主断后网压反馈 滤波后20091026atzy
extern Uint16 _BA_ENUDOUT;          				//郿岬缪故涑 0091026atzy
extern Uint16 _BA_ENUQOUT;          				//网侧q轴电压输出20091026atzy
extern Uint16 _BA_ETA1;													//最新故障时刻 网侧变流器A桥臂温度
extern Uint16 _BA_ETB1;													//最新故障时刻 网侧变流器B桥臂温度
extern Uint16 _BA_ETC1;													//最新故障时刻 网侧变流器C桥臂温度
extern Uint16 _BA_ETA2;													//最新故障时刻 电机侧变流器A疟畚露
extern Uint16 _BA_ETB2;													//最新故障时刻 电机侧淞髌鰾桥臂温
extern Uint16 _BA_ETC2;													//最鹿收鲜笨 电机侧变流器C桥臂温度
extern Uint16 _BA_EPIONU;													//最新故障时刻 网侧电压环输出
extern Uint16 _BA_EPIONID;													//最新故障时刻 网侧d轴电流环输出
extern Uint16 _BA_EPIONIQ;													//最新故障时刻 网侧q轴电流环输出
extern Uint16 _BA_EMEXI;													//最新故障时刻 机侧励磁理论计算值显示
extern Uint16 _BA_EPIOMID;													//最新故障时刻 机侧d轴电流环输
extern Uint16 _BA_EPIOMIQ;													//最新故障时刻 机侧q轴电流环输出
extern Uint16 _BA_ETLAC;													//最新故障时刻 网侧电感温度
extern Uint16 _BA_ETLDUDT;													//最新故障时刻 机侧电感温度
extern Uint16 _BA_ETSKIIP;													//最新故障时刻 SKIIP温
...
extern Uint16 _BA_TIME4_0;													//最新故障时间(秒)
extern Uint16 _BA_TIME4_1;													//最新故障时间(
extern Uint16 _BA_TIME4_2;													//最新故障时间(时)
extern Uint16 _BA_TIME4_3;													//最新故障时间(日)
extern Uint16 _BA_TIME4_4;													//最新故鲜奔 月)
extern Uint16 _BA_TIME4_5;													//最新故障时间(年)

extern Uint16 _BA_TIME3_0;													//第三次故障时间(秒)
extern Uint16 _BA_TIME3_1;													//第三次故障时间(分)
extern Uint16 _BA_TIME3_2;													//第三次故障时间(时)
extern Uint16 _BA_TIME3_3;													//第三次故障时间(日)
extern Uint16 _BA_TIME3_4;													//第三次故障时间(月)
extern Uint16 _BA_TIME3_5;													//第三次故鲜奔 年)

extern Uint16 _BA_TIME2_0;													//第二问障时间(秒)
extern Uint16 _BA_TIME2_1;													//第二次故障时间(分)
extern Uint16 _BA_TIME2_2;													//第二次故障奔?时)
extern Uint16 _BA_TIME2_3;													//第二次故障时间(日)
extern Uint16 _BA_TIME2_4;													//第二次故障时间(月)
extern Uint16 _BA_TIME2_5;													//第二次故障时?年)

extern Uint16 _BA_TIME1_0;													//第一次故障时间(秒)
extern Uint16 _BA_TIME1_1;													//第一次故障时间(分)
extern Uint16 _BA_TIME1_2;													//第一次故障时间(时)
extern Uint16 _BA_TIME1_3;													//第一次故障时间(日)
extern Uint16 _BA_TIME1_4;													//第一次故障时间(月)
extern Uint16 _BA_TIME1_5;													//第一次故障时间(年)

extern Uint16 _BA_TIME5_0;													//示波器故障时间(秒)
extern Uint16 _BA_TIME5_1;													//示波器故障时间(分)
extern Uint16 _BA_TIME5_2;													//示波器故障时间(时)
extern Uint16 _BA_TIME5_3;													//示波器故障时间(日)
extern Uint16 _BA_TIME5_4;													//示波器故障时间(月)
extern Uint16 _BA_TIME5_5;													//示波器故障时间(年)

//-------------------------------------运行监控变量-----------------------------------------------
extern Uint16 _BA_MIDRF;													//d轴电流指令(A)
extern Uint16 _BA_NIQRF;													//q轴电流指令(A)
extern Uint16 _BA_MIQRF;													//q轴电流指令(A)
extern Uint16 _BA_TOQRF;													//转矩指令(NM)
extern Uint16 _BA_AGLRF;													//无功角度指令(NM)
extern Uint16 _BA_TOQFB;													//转矩反馈(NM)
extern Uint16 _BA_URF;														//中间直流电压给定(V)
extern Uint16 _BA_UDC;														//中间直流电压(V)
extern Uint16 _BA_IA1;														//网郃相电流有效值(A)
extern Uint16 _BA_IB1;														//网侧B相电流有е?A)
extern Uint16 _BA_IC1;														//网侧C相电流有效值(A)
extern Uint16 _BA_IA2;														//电机侧A相电流有效值(A)
extern Uint16 _BA_IB2;														//电机侧B相电饔行е A)
extern Uint16 _BA_IC2;														//电机侧C相电流有效值(A)
extern Uint16 _BA_UAB1;													//网侧AB线电压有效值(V)
extern Uint16 _BA_UBC1;													//网侧BC线电压行е V)
extern Uint16 _BA_UAB2;													//电机侧AB线电压有效值(V)
extern Uint16 _BA_UBC2;													//电机侧BC线电流有效值(V)
extern Uint16 _BA_UAB0;												    	//主断前AB线电压有效值(V)
extern Uint16 _BA_UBC0;												    	//主断前BC线电流有效值(V)
extern Uint16 _BA_FREQ;													//电网频 hz)
extern Uint16 _BA_SPEED;													//电机转速(rpm)
extern Uint16 _BA_TA1;														//网侧变流器A桥畚露
extern Uint16 _BA_TB1;														//网侧变流鰾桥臂温度
extern Uint16 _BA_TC1;														//网侧变流器C桥臂温度
extern Uint16 _BA_TA2;														//电机侧变流器A桥臂温度
extern Uint16 _BA_TB2;														//电机侧变流器B桥臂温度
extern Uint16 _BA_TC2;														//电机侧变流器C桥臂温度
extern Uint16 _BA_PIONU;													//网侧电压环输出
extern Uint16 _BA_PIONID;													//网侧d轴电流环输出
extern Uint16 _BA_PIONIQ;													//网侧q轴电流环输出
extern Uint16 _BA_MEXI;													//
extern Uint16 _BA_PIOMID;													//机侧d轴电流环输出
extern Uint16 _BA_PIOMIQ;													//机侧q轴电流环输出
extern Uint16 _BA_GRDUD;													//传感器测量网压d
extern Uint16 _BA_GRDUQ;													//传感器测量网压q

//extern Uint16 _BA_STAUD;													//定子电压d MagnetCurve2013-12-13
extern Uint16 _BA_MC_IN;													//励磁校正输入 MagnetCurve2013-12-13
extern Uint16 _BA_MC_K;													//励磁校正输出 MagnetCurve2013-12-13
//extern Uint16 _BA_STAUQ;													//定子电压q MagnetCurve2013-12-13

extern Uint16 _BA_NPRID;													//传感器测量网压d
extern Uint16 _BA_NPRIQ;													//传感器测量网压q
//extern Uint16 _BA_EXCID;													//传感器测量网压d
//extern Uint16 _BA_EXCIQ;
extern Uint16 _BA_SCRIA;													//SCR电流a	20110906
extern Uint16 _BA_SCRIB;													//SCR电流b	20110906
extern Uint16 _BA_TLAC;
extern Uint16 _BA_TLDUDT;
extern Uint16 _BA_TNSKIIP;
extern Uint16 _BA_TMSKIIP;
extern Uint16 _BA_STAUABD;													//定子同步并网前定子前后电压差
extern Uint16 _BA_STAUBCD;													//定子同步并网前定子前后电压差
extern Uint16 _BA_STAIAC;													//定子线电流有效值显示
extern Uint16 _BA_STAIBA;													//定子线电流有效值显示
...
extern Uint16 _SCIB_BAUD;													//485波特率
extern Uint16 _SC_SKPTOV;
extern Uint16 _SC_FREQ1;
extern Uint16 _SC_FREQ2;

//-----------------------测试用变量-----------------------------------------------------------
extern int16 	zys[4];			//DA变量
extern Uint16 	daindex[4]; 	//BJTULVRT201204


//int32 	zys = 0;
//int16 	zys1,zys2,zys3,zys4;
extern Uint32 	i_cnt1;  //用于CCS波形显示
extern Uint32 	i_cnt2;  //用于CCS波形显示
extern Uint32 	i_cnt3;  //用于CCS波形显示
...
extern float   Te_feedback_lv;                    //实际转矩值反馈
