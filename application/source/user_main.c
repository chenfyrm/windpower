...
/*********************************************************************************************************
** 函数名称: CpuTimer0Isr
** 功能描述: 主定时器周期中断(PRD=30000 5kHz 200us 0.2ms)
** 输　入:
** 输　出:        
** 注  释: 
**-------------------------------------------------------------------------------------------------------
...
//		M_CHOPPER_EN();			//DC-CHOPPER驱动使能信号，拉高使能，开通关断还与底板滞环VDC2结果有关
		M_SetFlag(CL_CHOPEN);
	else
//		M_CHOPPER_DISEN();		//DC-CHOPPER驱动使能信号，拉低禁止
		M_ClrFlag(CL_CHOPEN);

	if((_COMMAND2&0x0800)!=0)	//11 5.11备用11-斩波测试 2013-12-6ZZJ
	{
...
//	if(NGS_Udq <= 0.94 * NGS_Udq_p_ex)							//20130301
	{
//		kq = 1.5 * (1 - NGS_Udq_p / GRD_UN);	//201205LVRTatZB	防止网压高时kq跳变20130124
...
//------------------------
	...
	if(M_ChkFlag(SL_LV_TEST)!=0)	//20120601night
	{
		...
		if(M_ChkCounter(MAIN_LOOP.cnt_lv_test2,DELAY_LVTEST2)>0)	//1s 2013-12-6 ZZJ 
		{
...
//			if(M_ChkFlag(SL_LV_STATE)==0 && (TRS_NPR_U.ampflt<(_stdby01 * NGS_Udq_p_ex)))		//20130309
			...
			else if(M_ChkFlag(SL_LV_STATE)==0 && (TRS_NPR_U.ampflt<(0.91 * NGS_Udq_p_ex)))//2013-12-6小于0.91延时1ms置位
			{
				lv_flag_flt++;
				if(lv_flag_flt>=5)
				{
...
//		if(M_ChkFlag(SL_LV_STATE)!=0 && (NGS_Udq_p > (0.92 * GRD_UN)))
		...
		{		
			if(M_ChkCounter(MAIN_LOOP.cnt_lv_rcv,DELAY_LVRCV)>0)			//10ms
			{
				...
				if(M_ChkCounter(MAIN_LOOP.cnt_lv_recmstop,50)<=0)		//误触发无需延时可立即重新置位20130303
				{
					if(NGS_Udq_epsilon < 2.5)	MAIN_LOOP.cnt_lv_test2 = 20000;						//应大于DELAY_LVTEST2
...
//				if(((NGS_Udq_p - NGS_Udq_p_lv) > 55) && (NGS_Udq_p_lv < (0.85 * NGS_Udq_p_ex)))	//3ph-75,2ph-50 20130205
				if(((NGS_Udq_p - NGS_Udq_p_lv) > 55) && ((NGS_Udq_p_lv < (0.82 * NGS_Udq_p_ex))||(NGS_Udq_epsilon_lv > 19)))	//3ph/2ph-50及以下 20130301
				{
...
//		LV_T2 = 4000;	
	...
	{
//		if(M_ChkFlag(SL_LV_TEST)!=0 && NGS_Udq_lv < (LV_A1 * GRD_UN))			//0.9 GRD_UN=额定电网相电压峰值 690->563V; 380->310V
		if(M_ChkFlag(SL_LV_TEST)!=0 && TRS_NPR_U.ampflt < (0.9 * GRD_UN))			//GRD_UN=额定电网相电压峰值 690->563V; 380->310V LVRTNR201103
...
//----------20130225采用电压跌落判断触发CROWBAR，加快投入速度-------------
	if(M_ChkFlag(SL_LV_STATE)!=0)
	{
	 if(M_ChkCounter(MAIN_LOOP.cnt_jgvdscron,30)<=0)		//30ms判断		20130205
	 {
...
//			if(NGS_Udq<(NGS_Udq_p_ex * 0.95))		//20130206 20%,35%,50%
			...
			{
				if(M_ChkFlag(SL_LV_NSTOP)==0)	M_SetFlag(SL_LV_ZDNSTOP);		//跌落瞬间主动封网侧脉冲20130202
			}
*/
//			if(NGS_Udq<(NGS_Udq_p_ex * 0.7))		//20130221 50%及以下所有工况
			if((NGS_Udq<(NGS_Udq_p_ex * 0.8))||(PRO.MPR_iac>150.0))		//20130228 50%及以下所有工况
//			if((NGS_Udq<(NGS_Udq_p_ex * 0.9))||(PRO.MPR_iac>1000.0))		//20130228 50%及以下所有工况
			{
				if(M_ChkFlag(SL_LV_MSTOP)==0)	M_SetFlag(SL_LV_ZDMSTOP);		//跌落瞬间主动封机侧脉冲20130202
			}
...
//-------------------绝对延时版本--LV MPR封脉冲控制-----------------
//	if(M_ChkFlag(SL_LV_MSTOP)==0 && M_ChkFlag(SL_LV_STATE)!=0 &&((PRO.MPR_iac>_SC_LVIAC2) || (PRO.udc>_SC_LVUDC1) || (M_ChkFlag(SL_LV_SCRRUNING)!=0) || (M_ChkFlag(SL_LV_VDSCRON)!=0)))   //20110326mpr软件过流或者Vdc过压或电网不平衡 201012
	if(M_ChkFlag(SL_LV_MSTOP)==0 && M_ChkFlag(SL_LV_STATE)!=0 &&((PRO.MPR_iac>_SC_LVIAC2) || (PRO.udc>_SC_LVUDC1) || (M_ChkFlag(SL_LV_SCRRUNING)!=0) || (M_ChkFlag(SL_LV_VDSCRON)!=0) || (M_ChkFlag(SL_LV_RECMSTOP)!=0) || (M_ChkFlag(SL_LV_ZDMSTOP)!=0)))   //20110326mpr软件过流或者Vdc过压或电网不平衡 20130225
	{
	...
	else if(M_ChkFlag(SL_LV_MSTOP)!=0)									//20110829尽快恢复励磁
...
//		if(M_ChkCounter(MAIN_LOOP.cnt_mpwmrestart,DELAY_MPWMRESTART)>=0)		//50ms
		...
		{
			if((PRO.MPR_iac<(_SC_LVIAC2-20)) && (PRO.udc<(_SC_LVUDC1-5)) && (M_ChkFlag(SL_LV_SCRRUNING)==0))		//20121210	小功率可以重启20130228
//			if(PRO.MPR_iac<(_SC_LVIAC2-20) && PRO.udc<(_SC_LVUDC1-5) && M_ChkFlag(SL_LV_SCRRUNING)==0 && M_ChkFlag(SL_UNBALANCE)==0)
//				M_ClrFlag(SL_LV_MSTOP);				  					//机侧LVRT及其过渡结束且不平衡消失，立即允许重启脉冲20110829
...
//				else if(((PRO.Pgactive_lv<600000) || (NGS_Udq_epsilon<27)) && ((_STDBY9&0x0080)!=0))	//116.7		20130303
				else if((NGS_Udq_epsilon<31) && (NGS_Udq_p>(NGS_Udq_p_ex * 0.70)))	//50%可启脉冲		20130308
...
//	if(M_ChkFlag(SL_LV_TEST)!=0 && (NGS_Udq < (0.9 * GRD_UN)))//20111201bjtu交大现行版本
...
//	if(M_ChkFlag(SL_LV_TEST)!=0 && M_ChkFlag(SL_LV_STATE)!=0 &&(NGS_Udq < (0.8 * GRD_UN)) && M_ChkFlag(SL_LV_MSTOP)==0 && M_ChkFlag(SL_LV_NSTOP)==0 && M_ChkFlag(SL_UNBALANCE)==0)//LVRTNR201103不平衡条件
	{
...
//-------------------------------------------2013-12-6由于_SC_LVIAC2从195改为190，所以_SC_LVIAC2-35改为_SC_LVIAC2-40	
	if(M_ChkFlag(SL_LV_NSTOP)==0 && M_ChkFlag(SL_LV_STATE)!=0 && ((PRO.NPR_iac>(_SC_LVIAC2-45)) || M_ChkFlag(SL_LV_ZDNSTOP)!=0 || M_ChkFlag(SL_LV_RECNSTOP)!=0)) 
	{
		M_SetFlag(SL_LV_NSTOP);
		M_ClrFlag(SL_LV_ZDNSTOP);		//20130202
		MAIN_LOOP.cnt_npwmrestart=0;									//LVRT网侧脉冲停止	

//		if(M_ChkFlag(SL_LV_RECNSTOP)!=0) //2013-12-6ZZJ取消恢复封网侧脉冲
...
//			 if(PRO.NPR_iac<(_SC_LVIAC2-20) && M_ChkFlag(SL_UNBALANCE)==0)
			 if(PRO.NPR_iac<(_SC_LVIAC2-45))				//20121226
				M_ClrFlag(SL_LV_NSTOP);				  							//机侧LVRT及其也黄胶庀В⒓丛市碇仄袈龀 0110829
		}
	}
	else MAIN_LOOP.cnt_npwmrestart=0;

//-------------------LV SCR电流到零判断（检测顺时值）-20110829HC----------------
...
//		if(M_ChkCounter(MAIN_LOOP.cnt_scrizero,_stdby06)>=0)	M_SetFlag(SL_LV_SCRIZERO);	  //201007BJTULVRT 通缌髋卸蟂CR是否都关断,DELAY_SCRIZERO延时靠实验确定 
		if(M_ChkCounter(MAIN_LOOP.cnt_scrizero,1)>=0)	M_SetFlag(SL_LV_SCRIZERO);	  //201007BJTULVRT 通缌髋卸蟂CR是否都关断,DELAY_SCRIZERO延时靠实验确定 
...
/*	if(((PRO.udc>1400) || ((PRO.udc>_SC_LVUDC2 || PRO.MPR_iac>(_SC_LVIAC2+15)) && M_ChkFlag(SL_LV_STATE)!=0) || M_ChkFlag(SL_LV_VDSCRON)!=0)&& M_ChkFlag(SL_LV_SCRON)==0) 
	...
	{
		M_SetFlag(SL_LV_SCRON);							//开启SCR动作指令
		MAIN_LOOP.cnt_lv_scron=0;						//重要：保证下次SCR开通依然延时_SC_SCRON2010115修改
		CNT_SCRON++;		//20120602					
	}
	else if(PRO.udc< (_SC_LVUDC2-30))
	{
		M_ClrFlag(SL_LV_SCRON);							//关断SCR指令	
	}
*/
//--------20130225--------------
	if(M_ChkFlag(SL_LV_SCRDONE)==0)		//20130221
	{
		if(((PRO.udc>1400) || ((PRO.udc>_SC_LVUDC2 || PRO.MPR_iac>(_SC_LVIAC2)) && M_ChkFlag(SL_LV_STATE)!=0) || M_ChkFlag(SL_LV_VDSCRON)!=0)&& M_ChkFlag(SL_LV_SCRON)==0) //2013-12-6 ZZJ首次投入1300A
		{
			M_SetFlag(SL_LV_SCRON);							//开启SCR动作指令
			MAIN_LOOP.cnt_lv_scron=0;						//重要：保证下次SCR开通依然延时_SC_SCRON2010115修改
		}
	...
	{
		if(((PRO.udc>1400) || ((PRO.udc>1250 || PRO.MPR_iac>(_SC_LVIAC2+15)) && M_ChkFlag(SL_LV_STATE)!=0))&& M_ChkFlag(SL_LV_SCRON)==0)//2013-12-6 ZZJ 
		{
			M_SetFlag(SL_LV_SCRON);							//开启SCR动作指令
			MAIN_LOOP.cnt_lv_scron=0;						//重要：保证下次SCR开通依然延时_SC_SCRON2010115修改
		}
...
//-------------------LV SCR控制-采用后须将机侧临时封脉冲的SL_LV_SCRON换成SL_LV_SCRRUNING--------------------------------------
//	if(M_ChkFlag(SL_LV_SCRON)!=0 || M_ChkFlag(SL_LV_SCRKEEPON)!=0) //20120602											//要求Crowbar开通
	if(M_ChkFlag(SL_LV_SCRON)!=0)											//要求Crowbar开通
	{		
		M_ACROWBARON();								 						//SCR开通
		M_SetFlag(SL_LV_SCRRUNING);											//SCR开通标志位
		M_SetFlag(SL_DISPLAY6);													
	}
	else
	{
//---------分跌落深度和类型切出Crowbar20121210---------20130225

		if(M_ChkFlag(SL_LV_SCRRUNING)!=0)
		{
//--------20130221--------
//			if(M_ChkCounter(MAIN_LOOP.cnt_lv_scroff3,2)>0)			//投入scr后2ms判断
			if(M_ChkCounter(MAIN_LOOP.cnt_lv_scroff3,_stdby04)>0)			//投入scr后2ms判断	20130309
			{
//				if(M_ChkFlag(SL_LV_SCRDONE)!=0)
//				if((scroff1==1) || (M_ChkFlag(SL_LV_SCRDONE)!=0) || ((NGS_Udq_epsilon > 18) && (PRO.Pgactive_lv<600000) && (NGS_Udq_p>(0.66 * NGS_Udq_p_ex))))	//小功率35%-2ph及以上只投2-5ms	20130303
/*				if((scroff1==1) || (M_ChkFlag(SL_LV_SCRDONE)!=0) || ((NGS_Udq_epsilon > 18) && (PRO.Pgactive_lv<600000) && (NGS_Udq_p>(0.55 * NGS_Udq_p_ex))))	//小功率20%-2ph及以上只投2-5ms	20130308
				{
					scroff1 = 1;
					M_ACROWBAROFF();
					if(M_ChkFlag(SL_LV_SCRIZERO)!=0)							//检测到SCR电鹘咏
					{
						M_ClrFlag(SL_LV_SCRRUNING);								//结束SCR动作进程
						MAIN_LOOP.cnt_lv_scroff3=0;								//重要：保证下次SCR开通依然延时_SC_SCRON2010115修改	
						scroff1 = 0;
					}
				}
*///--------20130309小功率20%再次投入--------
				if((M_ChkFlag(SL_LV_SCRDONE)!=0) || (PRO.Pgactive_lv<600000))	//小功率20%-2ph及以上只投2-5ms	20130308
				{
					M_ACROWBAROFF();
					if(M_ChkFlag(SL_LV_SCRIZERO)!=0)							//检测到SCR电鹘咏
					{
...
//							M_ACROWBARON();
							if((_STDBY9&0x0004)!=0)		M_ACROWBARON();		//116.2	只有20-2ph小风才能再次投入	20130314
							scroff1 = 1;
						}
						else
						{
...
//				if(((NGS_Udq_p>280) && (NGS_Udq_epsilon < 15)) || (M_ChkFlag(SL_LV_SCROFF2)!=0))				//跌幅深度低于50%	20130116
				if(((NGS_Udq_p>190) && (NGS_Udq_epsilon < 15)) || (M_ChkFlag(SL_LV_SCROFF2)!=0))				//35%及以上情况（除35%大风）	20130221
				{
...
*///20130309取消,恢复时注意恢复定时器
			if(M_ChkCounter(MAIN_LOOP.cnt_lv_scron,_SC_TSCRON)>0)			//SCR开通一个_SC_SCRON固定奔洌僮髌骺傻
			{								
				...
				else if(NGS_Udq_epsilon > 26)		//涵盖20%小功率及35%大小功率20130222
				{
...
//	PRO.Pgactive = Te_feedback * PRO.speedflt * PAI * 0.03333333;	//P=T*2PIE*n/60 201005atcpc
	...
	DataFilter(0.99,&PRO.Pgactive,PRO.Pgactive2); 		//注：改滤波 00us/2.5kHz执行一次。定子侧电流反馈值滤波,Ts=200us,滤掉开关频率次0.97
...
//-----------------------中间直流斩波电流过流判断-BJTULVRT201204----------------------------------------------------------
		if(M_ChkFlag(SL_LV_STATE)==0)
		{
			if(M_ChkCounter(MAIN_LOOP.cnt_idcovdelay,DELAY_IDCOVDELAY)>=0)
			{
				...
				{
					if(M_ChkCounter(MAIN_LOOP.cnt_idcov,DELAY_IDCOV)>=0)	M_SetFlag(SL_IDCOV);		//置定子电流过流标志位201205LVRTatZB20120531
				}
				else if((M_ChkFlag(SL_IDCOV)!=0)&&((M_ChkFlag(SL_LV_STATE)==0)||((_COMMAND2&0x0002)==0))&&(PRO.CHOP_idc<5)) 
				{
					M_ClrFlag(SL_IDCOV);									//清chopper电流故障标净
			...
			{
				MAIN_LOOP.cnt_idcov = 0;
				M_ClrFlag(SL_IDCOV);									//清chopper电流故障标志位
			}
		}
		else
		{
...
//				_BA_ETLAC  = (int16)SCI_canopen.cnt_heartbeat;             //网侧绺形露 2014-05-06LJD临时修改CANopen
				...
				_BA_ENPRUQ2 = (int16)TRS_NGS_U.qflt;          			//q轴主断后网压反馈 瞬ê 0091026atzy
...
//1s
	...
	else
	{
		if(ET_SUM1>et_gate)
		{
			M_SetFlag(SL_SIAC1);
			ET_SUM1=et_gate;
//			M_SetFlag(SL_ERRDATASAVE);	//触发外部RAM数据转 0091109atzy
		}
...
//ADctl_zl_end
    ...
    if(MAIN_LOOP.cnt_nprcmd!=65535)       	MAIN_LOOP.cnt_nprcmd++;         //上位机允许NPR发脉冲邮 s，防止误操作
...
//	_BA_TLAC  = (int16)SCI_canopen.cnt_heartbeat;	    //网侧电感温度 2014-05-06LJD临时修改为CANopen
	...
	_BA_MEXI   = (int16)(DISP.mexi  * 10);			    // 6.23change_zl改为1恍∈ 改为励磁电流理论值显示cpc
	...
	if(M_ChkFlag(SL_PC_ERRDATACLR)!=0)						//PC指令外部RAM数据释放
	{
  	...
  	if(*(ERROR_RAMEND + 1) != 0x55AA)
  	{
		...
		if(M_ChkFlag(SL_RAMERRORSAVE)==0)  										//有故障之后延时2s后停止数据存储
		{
			...
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 4 + ERROR_RAMDATA_POS)) = (int16)(AD_OUT_NPR_I.c*10);		//4=NPR-C电流
			...
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 7 + ERROR_RAMDATA_POS)) = (int16)(TRS_NGS_U.d*10);			//7=筪轴分量ed滤波前
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 8 + ERROR_RAMDATA_POS)) = (int16)(TRS_NGS_U.q*10);			//8=网压q轴分量eq滤波前
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 9 + ERROR_RAMDATA_POS)) = (int16)(TRS_NGS_U.dflt*10);		//9=网压d轴分量ed滤波后
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 10 + ERROR_RAMDATA_POS)) = (int16)(TRS_NGS_U.qflt*10);		//10=网压q轴分量eq滤波后
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 11 + ERROR_RAMDATA_POS)) = (int16)(PI_NPR_Id.reference*10);//11=网侧Id指令
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 12 + ERROR_RAMDATA_POS)) = (int16)(TRS_NPR_I.dflt*10);		//12=网侧Id反
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 13 + ERROR_RAMDATA_POS)) = (int16)(PI_NPR_Id.out*10);		//13=网侧IdPI输出
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 14 + ERROR_RAMDATA_POS)) = (int16)(TRS_NPR_U.d*10);		//14=网侧Ud输出
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 15 + ERROR_RAMDATA_POS)) = (int16)(PI_NPR_Iq.reference*10);//15=网侧Iq指令
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 16 + ERROR_RAMDATA_POS)) = (int16)(PI_NPR_Iq.feedback*10);	//16=网侧Iq反馈
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 17 + ERROR_RAMDATA_POS)) = (int16)(PI_NPR_Iq.out*10);		//17=网侧IqPI输出
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 18 + ERROR_RAMDATA_POS)) = (int16)(TRS_NPR_U.q*10);		//18=网侧Uq输出
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 19 + ERROR_RAMDATA_POS)) = (int16)(CAP4.mprtrstheta*1000);	//19=MPR定向角度
			...
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 23 + ERROR_RAMDATA_POS)) = (int16)(CAP4.freqtmp*10);		//23=实际实时网频
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 24 + ERROR_RAMDATA_POS)) = (int16)PRO.speed;				//24=电机转速

			*(ERROR_RAMEND) = ERROR_RAMDATA_POS;															//鼻笆荽娲⑽恢米
...
//----------LVRT--201205LVRTatZB
			...
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 19 + ERROR_RAMDATA_POS)) = (int16)(NGS_Udq_epsilon* 10);	//19=MPR定向角度
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 20 + ERROR_RAMDATA_POS)) = (int16)(AD_OUT_MPR_I.a*10);		//20=MPR-A电流
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 21 + ERROR_RAMDATA_POS)) = (int16)(AD_OUT_MPR_I.b*10);		//21=MPR-B电流
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 22 + ERROR_RAMDATA_POS)) = (int16)(AD_OUT_MPR_I.c*10);		//22=MPR-C电流
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 23 + ERROR_RAMDATA_POS)) = (int16)(M_ChkFlag(SL_LV_STATE) * 10);			//23=**********延时恢复的电网残压**********
			*(ERROR_RAMSTART+((Uint32)RAM_BIAS * 24 + ERROR_RAMDATA_POS)) = (int16)PRO.speed; 				//24=电机转速
...
/*
		...
		{
			*(BANK_RAMSTART+ BANK_RAMDATA_POS) = (int16)(CAP4.nprtrstheta* 1000);							//0=网侧定向角度
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 1 + BANK_RAMDATA_POS)) = (int16)(CAP4.mprtrstheta* 1000);	//1=机侧定向角度
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 2 + BANK_RAMDATA_POS)) = (int16)(QEPDATA.rotposdisp* 1000);	//2=转子角
			...
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 7 + BANK_RAMDATA_POS)) = (int16)AD_OUT_UDC;					//7=中间直流电压
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 8 + BANK_RAMDATA_POS)) = (int16)(TRS_NGS_U.d* 10);			//8=电网d轴电压分量滤波前
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 9 + BANK_RAMDATA_POS)) = (int16)(TRS_NGS_U.q* 10);			//9=电网q轴电压分量滤波前
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 10 + BANK_RAMDATA_POS)) = (int16)(AD_OUT_NPR_I.a* 10);		//10=网侧A相电流
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 11 + BANK_RAMDATA_POS)) = (int16)(AD_OUT_NPR_I.b* 10);		//11=网侧B相电流
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 12 + BANK_RAMDATA_POS)) = (int16)(AD_OUT_NPR_I.c* 10);		//12=网侧C相电流
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 13 + BANK_RAMDATA_POS)) = (int16)(AD_OUT_MPR_I.a* 10);		//13=机侧A相电流
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 14 + BANK_RAMDATA_POS)) = (int16)(AD_OUT_MPR_I.b* 10);		//14=机侧B相电流
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 15 + BANK_RAMDATA_POS)) = (int16)(AD_OUT_MPR_I.c* 10);		//15=机侧C相电流
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 16 + BANK_RAMDATA_POS)) = (int16)(CAP4.freqtmp* 10);			//16=电网实时频率
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 17 + BANK_RAMDATA_POS)) = (int16)PRO.speed;					//17=电机转速
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 18 + BANK_RAMDATA_POS)) = (int16)(PI_NPR_Id.reference* 10);	//18=网侧电压环输出
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 19 + BANK_RAMDATA_POS)) = (int16)(PI_NPR_Id.out* 10);		//19=网侧d岬缌骰肥涑
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 20 + BANK_RAMDATA_POS)) = (int16)(PI_NPR_Iq.out* 10);		//20=网侧q轴电流环输出
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 21 + BANK_RAMDATA_POS)) = (int16)(DM_imrd* 10);				//21=机侧广义励磁电流
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 22 + BANK_RAMDATA_POS)) = (int16)(RUN.mpridrf* 10);				//22=机侧励磁电流指令
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 23 + BANK_RAMDATA_POS)) = (int16)(PI_MPR_Id.out* 10);		//23=机侧d轴电流环输出
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 24 + BANK_RAMDATA_POS)) = (int16)(PI_MPR_Iq.out* 10);		//24=机侧q轴电流环输出
...
//			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 5 + BANK_RAMDATA_POS)) = (int16)(M_ChkFlag(SL_LV_SCRRUNING)* 10);		//5=定子滤波AB线电压
//			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 6 + BANK_RAMDATA_POS)) = (int16)(M_ChkFlag(SL_LV_SCRIZERO)* 10);		//6=定子滤波BC线电压
			...
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 9 + BANK_RAMDATA_POS)) = (int16)(M_ChkFlag(SL_LV_MSTOP)* 10);			//9=电网q轴电压分柯瞬ㄇ
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 10 + BANK_RAMDATA_POS)) = (int16)(M_ChkFlag(SL_LV_NSTOP)* 10);		//10=网侧A相电流
...
//			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 19 + BANK_RAMDATA_POS)) = (int16)(M_ChkFlag(SL_LV_SCRKEEPON)* 10);		//19=定子缌鰽相BJTULVRT201204
			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 19 + BANK_RAMDATA_POS)) = (int16)(M_ChkFlag(SL_LV_SCRDONE)* 10);		//19=定子缌鰽相BJTULVRT20130221
...
//			*(BANK_RAMSTART+((Uint32)RAM_BIAS * 24 + BANK_RAMDATA_POS)) = (int16)(PHI_DATA_M.Iq);		//24=SCR支路电流 BJTULVRT201204
			...
			*(BANK_RAMEND) = BANK_RAMDATA_POS;																//鼻数据存⑽恢米
