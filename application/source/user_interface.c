...
void Ad8364Ctrl(void)
{
	...
	//------------------------------数据读取(共18路)---------------------------------
	//该组AD数据是100us之前的结果
	AD.dat[0] = *AD_ASTART;	       // UACA2， 电机定子侧电压Ubc   ，大滤波
	AD.dat[1] = *(AD_ASTART+1);    // AMUX，慢速信号，16选1过来的信号，2电感温度、7模块温度、2crowbar电流、2定子电流、2模块最大电流
	AD.dat[2] = *(AD_ASTART+2);    //UACB1， chopper电流idc	20130805
	//	AD.dat[3] = *(AD_ASTART+3);    // Ic ，备用，SKIIP电流反馈
	//	AD.dat[4] = *(AD_ASTART+4);    // UACB2，备用
	AD.dat[5] = *(AD_ASTART+5);    // IB3，MPR.ic, 机侧变流器MPR的电流

	AD.dat[6]  = *AD_BSTART;       // UACB3，与UACA3同一个采样，NGF.Uab ，大滤波
	AD.dat[7]  = *(AD_BSTART+1);   // IB2，MPR.ib，机侧变流器NPR
	AD.dat[8]  = *(AD_BSTART+2);   // UACB4，与UACA1同一个采样，NGF.Ubc,大滤波
	AD.dat[9]  = *(AD_BSTART+3);   // IB1，MPR.ia，机侧变流器NPR的电流
	AD.dat[10] = *(AD_BSTART+4);   // VDC1，Udc 中间直流电压
	AD.dat[11] = *(AD_BSTART+5);   // IA3，NPR.ic，网侧变流器NPR的电流

	//	AD.dat[12] = *AD_CSTART;       // Vdc2 ，备用
	AD.dat[13] = *(AD_CSTART+1);   // IA2，NPR.ib，网侧变流器NPR的电流
	AD.dat[14] = *(AD_CSTART+2);   // UACA3，NGS.Uab 网侧电压Uab
	AD.dat[15] = *(AD_CSTART+3);   // IA1，NPR.ia，网侧变流器NPR的电流
	AD.dat[16] = *(AD_CSTART+4);   // UACA4，STA.Uab 电机定子侧电压Uab，大滤波
	AD.dat[17] = *(AD_CSTART+5);   // UACA1，NGS.Ubc 网侧电压Ubc
	//----------------------------数据读取结束------------------------------
	...
	DataFilter(0.9999,&MEAN_DATA.iac3,abs(AD_OUT_STA_I.ac)); //定子侧电流	平均值滤波10S
	DataFilter(0.9999,&MEAN_DATA.icb3,abs(AD_OUT_STA_I.cb)); //定子侧电流	平均值滤波10S
	DataFilter(0.9999,&MEAN_DATA.ia3,abs(AD_OUT_STA_I.a)); //定子侧电流	平均值滤波10S
	DataFilter(0.9999,&MEAN_DATA.ib3,abs(AD_OUT_STA_I.b)); //定子侧电流	平均值滤波10S
	DataFilter(0.9999,&MEAN_DATA.ic3,abs(AD_OUT_STA_I.c)); //定子侧电流	平均值滤波10S
	...
	PRO.MPR_iac = temp * 100 / MPR_IACN;		//转子电流瞬时最大值，基准电流的百分值
	...
	//---------------------平均值滤波------------------------------------------------------------------
	DataFilter(0.9999,&MEAN_DATA.ia1,abs(AD_OUT_NPR_I.a)); //网侧电流	平均值滤波10S
	DataFilter(0.9999,&MEAN_DATA.ib1,abs(AD_OUT_NPR_I.b)); //网侧电流	平均值滤波10S
	DataFilter(0.9999,&MEAN_DATA.ic1,abs(AD_OUT_NPR_I.c)); //网侧电流	平均值滤波10S

	DataFilter(0.9999,&MEAN_DATA.ia2,abs(AD_OUT_MPR_I.a)); //机侧电流	平均值滤波10S
	DataFilter(0.9999,&MEAN_DATA.ib2,abs(AD_OUT_MPR_I.b)); //机侧电流	平均值滤波10S
	DataFilter(0.9999,&MEAN_DATA.ic2,abs(AD_OUT_MPR_I.c)); //机侧电流	平均值滤波10S

	DataFilter(0.9999,&MEAN_DATA.uab1,abs(AD_OUT_NGS_U.ab)); //网侧电压	平均值滤波10S
	DataFilter(0.9999,&MEAN_DATA.ubc1,abs(AD_OUT_NGS_U.bc)); //网侧电压	平均值滤波10S

	DataFilter(0.9999,&MEAN_DATA.uab2,abs(AD_OUT_STA_U.ab)); //定子侧电压	平均值滤波10S
	DataFilter(0.9999,&MEAN_DATA.ubc2,abs(AD_OUT_STA_U.bc)); //定子侧电压	平均值滤波10S

	DataFilter(0.9999,&MEAN_DATA.ua1,abs(AD_OUT_NGS_U.a)); //网侧电压	平均值滤波10S
	DataFilter(0.9999,&MEAN_DATA.ub1,abs(AD_OUT_NGS_U.b)); //网侧电压	平均值滤波10S
	DataFilter(0.9999,&MEAN_DATA.uc1,abs(AD_OUT_NGS_U.c)); //网侧电压	平均值滤波10S

	//--------------求网压和定子电压差值平均值------------------------------------------
	AD_OUT_STAD_U.ab = AD_OUT_NGF_U.ab - AD_OUT_STA_U.ab;
	AD_OUT_STAD_U.bc = AD_OUT_NGF_U.bc - AD_OUT_STA_U.bc;

	DataFilter(0.9999,&MEAN_DATA.uab_d,abs(AD_OUT_STAD_U.ab));  //差值平均值    平均值滤波10S
	DataFilter(0.9999,&MEAN_DATA.ubc_d,abs(AD_OUT_STAD_U.bc));  //差值平均值	平均值滤波10S
	DataFilter(0.997,&MEAN_DATA.ubc_t,abs(AD_OUT_STAD_U.bc));  //差值平均值	平均值滤波1S	//20130801
